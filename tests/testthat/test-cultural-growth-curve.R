# test-cultural-growth-curve.R — H5: growth curve and valence ODE fit
#
# Tests that cultural accumulation shows:
#   1. Positive acceleration (quadratic coefficient > 0)
#   2. valence ODE confirms generative regime (r_eff > 0)
#   3. valence ODE beats quadratic (Gabora) model
#
# @dft A1, A6
...
test_that("H5: valence ODE confirms generative regime (r_eff > 0)", {
  data <- load_uspto_patents()$data
...
test_that("H5: valence ODE beats quadratic (Gabora) model", {
  data <- load_uspto_patents()$data
  t <- data$year - min(data$year)
  B <- data$cumulative_patents
  result <- fit_valence_ode_models(t, B)
  ...
  # When it does, AIC is NA — valence ODE wins by default
  # When it doesn't, valence ODE should have lower AIC
  if (is.na(result$values$quad_aic)) {
    # Quadratic diverged — valence ODE wins by default
