...
test_that("relaxation_analytical: returns rho0 at t = 0", {
  result <- relaxation_analytical(
    t = 0, rho0 = 1.0, k1 = 0.5, k2 = 0.02,
    rho1 = 0.3, rho2 = 0.3
  )
...
test_that("relaxation_analytical: approaches equilibrium as t → ∞", {
  ...
  result <- relaxation_analytical(t,
    rho0 = 1.0, k1 = 0.5, k2 = 0.02,
    rho1 = 0.3, rho2 = 0.3
  )
  # Equilibrium value: k1*rho1 + k2*rho2, divided by k1 + k2
...
test_that("relaxation_analytical: handles k2 = 0 (single channel)", {
  result <- relaxation_analytical(
    t = c(0, 1, 10), rho0 = 1.0, k1 = 0.5, k2 = 0,
    rho1 = 0.3, rho2 = 0.3
  )
...
test_that("relaxation_analytical: faster k1 + k2 gives faster decay", {
  t <- 10
  slow <- relaxation_analytical(t,
    rho0 = 1.0, k1 = 0.1, k2 = 0.01,
    rho1 = 0.3, rho2 = 0.3
  )
  fast <- relaxation_analytical(t,
    rho0 = 1.0, k1 = 1.0, k2 = 0.1,
    rho1 = 0.3, rho2 = 0.3
  )
  expect_lt(fast, slow)
})
...
test_that("relaxation_analytical: different rho1, rho2 produce different equilibrium", {
  t <- 1000
  a <- relaxation_analytical(t,
    rho0 = 1.0, k1 = 0.5, k2 = 0.02,
    rho1 = 0.3, rho2 = 0.3
  )
  b <- relaxation_analytical(t,
    rho0 = 1.0, k1 = 0.5, k2 = 0.02,
    rho1 = 0.5, rho2 = 0.1
  )
...
test_that("relaxation_simulate: returns expected structure", {
  result <- relaxation_simulate(
    times = seq(0, 100, 1), rho0 = 1.0,
    k1 = 0.5, k2 = 0.02, rho1 = 0.3, rho2 = 0.3
  )
  expect_named(result, c(
    "times", "rho", "rho0", "k", "rho_star",
...
test_that("relaxation_simulate: analytical matches closed form", {
  times <- seq(0, 50, 0.5)
  result <- relaxation_simulate(times,
    rho0 = 1.0, k1 = 0.5, k2 = 0.02,
    rho1 = 0.3, rho2 = 0.3
  )
...
test_that("relaxation_simulate: errors on fewer than 2 time points", {
  expect_error(relaxation_simulate(
    times = 1, rho0 = 1.0, k1 = 0.5,
...
test_that("generate_relaxation_data: returns expected structure", {
  d <- generate_relaxation_data(seq(0, 10, 0.1),
    rho0 = 1.0,
    k1 = 0.5, k2 = 0.02, rho1 = 0.3, rho2 = 0.3,
...
test_that("generate_relaxation_data: rho_true is deterministic", {
  d1 <- generate_relaxation_data(seq(0, 10, 0.1),
    rho0 = 1.0,
    k1 = 0.5, k2 = 0.02, rho1 = 0.3, rho2 = 0.3,
    noise_sd = 0, seed = 42
  )
  d2 <- generate_relaxation_data(seq(0, 10, 0.1),
    rho0 = 1.0,
    k1 = 0.5, k2 = 0.02, rho1 = 0.3, rho2 = 0.3,
    noise_sd = 0, seed = 42
  )
...
test_that("relaxation_phase_analysis: returns expected structure", {
  ...
  expect_named(analysis, c(
    "phase1_rate", "phase2_rate", "rate_ratio",
