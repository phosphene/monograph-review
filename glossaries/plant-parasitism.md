...
> Domain glossary for the framework under review (monograph review).
> Defines terms mechanically — by how they work in the biology, not by analogy.
> All definitions are grounded in the foundry's baseline tests (T1–T2, T6, formal model, L3).
...
**Why T1 is non-discriminating despite the strong p-value.** Relaxed selection (see §5) predicts the same gradient. The PGLS result *corroborates* the framework (the predicted direction holds) but does not *discriminate* it from the competitor. This is honestly tagged in the oracle as `distinguishes_from_competitor: false`.
...
**The competitor prediction.** For the T1/T2 gradient tests, relaxed selection predicts the same empirical pattern as the framework: deeper parasitism → smaller plastome. The mechanism differs:
- **The framework**: capacity reallocation driven by integration depth — deeply integrated genes are retained because other functions depend on them, regardless of the gene's own function.
...
**Why T1/T2 cannot distinguish them.** Both mechanisms predict a monotonic gradient: species with less photosynthesis need fewer photosynthetic genes. The framework's prediction adds a *specific ordering* of gene loss (deeply integrated → retained longer), but the T1/T2 tests only measure total plastome size, not which genes are lost. A species at level 3 could lose 30 kb via either mechanism — the size data alone cannot tell which.

**Where they can be distinguished.** The discriminating tests are:
- **T6 (gene-loss ordering):** The framework predicts loss order by integration depth (ρ ≈ 0.96); relaxed selection predicts loss order by gene function (photosynthesis-specific genes go first, regardless of integration depth). The two predictions sometimes coincide (ndh is both shallow-integrated and photosynthetic) but diverge for multi-functional genes (rpl/rps are integration-deep but not photosynthesis-specific — the framework predicts retention, relaxed selection predicts loss if they serve no plastid-translation function).
- **Formal model GLM:** The framework predicts dep > 0 (higher dependency → higher retention) and para < 0 (deeper parasitism → lower retention). Relaxed selection predicts dep ≤ 0 (dependency should not matter — functionlessness, not integration, drives loss) and para < 0 (same gradient). The corrected GLM finds dep = +0.84 (p = 0.0008), which supports the framework over relaxed selection.
- **T3 (biphasic kinetics):** The framework predicts a threshold gate (fast loss of unprotected traits, then stasis). Relaxed selection predicts constant-rate loss (exponential decay of all unselected genes at the same rate). The biphasic shape distinguishes them — but T3 is currently misspecified for the available data (within-lineage test needed, not cross-sectional).
...
**The pattern the framework predicts.** Higher dependency (dep) → higher retention, especially at deeper parasitism. The mean retention by dependency (collapsed across species) is roughly monotonic:
...
The positive sign means: for each unit increase in dep score, the log-odds of retention increase by 0.84 — deeply integrated genes are retained more. This is the central empirical prediction of the framework under review. The corrected GLM (after the data-flattening bug fix) confirms this prediction.
...
*Written for the foundry review. All values from `baseline/oracle.yml` (v2026-08-16).*