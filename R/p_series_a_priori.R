...
p1_buchnera_two_component <- function(scores, seed = 42L) {
  ...
  mB <- glm(retained ~ dependency_score, data = d, family = binomial)
  ...
  mA <- glm(retained ~ essential_flag, data = d, family = binomial)
  # Two-component model
  mAB <- glm(retained ~ dependency_score + essential_flag, data = d, family = binomial)

  pB <- summary(mB)$coefficients["dependency_score", "Pr(>|z|)"]
  pA <- summary(mA)$coefficients["essential_flag", "Pr(>|z|)"]
  pAB <- summary(mAB)$coefficients["dependency_score", "Pr(>|z|)"]
  pAB_ess <- summary(mAB)$coefficients["essential_flag", "Pr(>|z|)"]

  r2_B <- 1 - as.numeric(logLik(mB)) / as.numeric(logLik(glm(retained ~ 1, data = d, family = binomial)))
  r2_AB <- 1 - as.numeric(logLik(mAB)) / as.numeric(logLik(glm(retained ~ 1, data = d, family = binomial)))
  ...
  aa_genes <- c(
    ...
    "carA","carB","purA","purB","purC","purD","purE","purF","purG","purH","purK","purL","purM","purN","purR","purT","purU",
    "guaA","guaB",
    "upp","udp","udk",
    "deoA","deoB","deoC","deoD",
    "tktA","tktB","talA","talB",
  ...
  dLM <- d[d$low_mismatch == 1, ]
  rho_LM <- NA
  p_rho_LM <- NA
  if (nrow(dLM) >= 3 && length(unique(dLM$retained)) > 1) {
    ct <- cor.test(dLM$dependency_score, dLM$retained, method = "spearman")
    rho_LM <- ct$estimate; p_rho_LM <- ct$p.value
  }

  list(
    values = list(
      beta_B = unname(coef(mB)["dependency_score"]),
      p_B = pB,
      beta_A = unname(coef(mA)["essential"]),
      p_A = pA,
      beta_B_AB = unname(coef(mAB)["dependency_score"]),
      p_B_AB = pAB,
      p_A_AB = pAB_ess,
      pseudo_r2_B = r2_B,
      pseudo_r2_AB = r2_AB,
    ...
    metadata = list(
      ...
      components = "A: functional-category demand proxy (amino-acid/cofactor biosynthesis); B: iJO1366 dependency_score",
...
p2_ltee_niche_fba <- function(scores, seed = 42L) {
  ...
  dLM <- d[d$niche_mismatch == 0 & !is.na(d$string_ppi_degree), ]
  rho_B <- NA; p_rho_B <- NA
  if (nrow(dLM) >= 3) {
    ct <- cor.test(dLM$string_ppi_degree, dLM$first_gen, method = "spearman")
    rho_B <- ct$estimate; p_rho_B <- ct$p.value
  }

  # Component B on full set (T7-v2 replication): degree vs timing
  dF <- d[!is.na(d$string_ppi_degree), ]
  rho_full <- NA; p_rho_full <- NA
  if (nrow(dF) >= 3) {
    ct <- cor.test(dF$string_ppi_degree, dF$first_gen, method = "spearman")
    rho_full <- ct$estimate; p_rho_full <- ct$p.value
  }
...
p3_plastid_erosion_order <- function(scores, seed = 42L) {
  ...
  sp_rhos <- sapply(split(d, d$species), function(sub) {
    if (length(unique(sub$dependency_score)) < 2 || length(unique(sub$retention)) < 2) return(NA)
...
p4_echolocation_centrality <- function(symbols, centralities, sym_to_prot, seed = 42L) {
  ...
  res <- sapply(metrics, function(met) {
    ...
    c(conv_mean = mean(ca), cons_mean = mean(ba), p_conv_less = pv,
