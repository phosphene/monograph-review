...
Does the C4 photosynthesis syndrome show the VI integration-depth signature — *function converges (invariant across origins, high integration), mechanism diverges (varying across origins, low integration)* — as coded by the C4 literature itself?

## Design (a priori, non-circular)

- **Hierarchy:** Christin & Osborne (2014, *New Phytologist* 204:1146) Table 1 provides a six-level phenotypic deconstruction: **Niche > Physiology > Function > Character > Characteristic > Component** — an integration-depth hierarchy defined by the C4 field, not by VI.
...
The C4 literature independently formalizes an integration-depth hierarchy (Table 1) and independently codes the shared/varying structure exactly as VI predicts: invariant elements (C4 physiology, its tissue-level functions) sit at the top; varying elements (characters, characteristics, components) sit at the bottom. The alignment is perfect in direction (ρ = +0.866, 2/2 invariant in upper half, 0 in lower).
...
3. **Qualitative claims, quantitative direction.** P5 shows the literature's framework aligns with VI; it does not measure an effect size. Its role in the monograph is corroboration of the integration-depth pattern across kingdoms (mammal gene expression, eubacterial genomes, plastid genomes, plant physiology).