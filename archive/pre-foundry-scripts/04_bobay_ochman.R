...
# Tests the framework prediction: Ne (as proxy for drift barrier) should correlate
...
cat(sprintf("Direction: %s (log-log slope = %.3f)\n",
            ...
            coef(mod_ne_pan)[2]))
cat("the framework prediction: drift barrier (Ne) shapes pan-genome architecture\n")
cat("=============================\n")
