...
| [`simulation-genealogy-and-practice.md`](simulation-genealogy-and-practice.md) | The genealogy of fabricated data as an instrument — Fisher randomization → Monte Carlo → Boxian checking → bootstrap → null models → ABC/synthetic likelihood → Simulation-Based Calibration → designed simulation studies (ADEMP) — the best-practice commitments it entails as applicable to R, and an honest measurement of where the foundry's own practice sits in that lineage. Working note; the Wisdom symmetry ties it to the review's Darwin–Wisdom tradition. |
| [`self-calibration-tickets.md`](self-calibration-tickets.md) | The work program for turning the simulation genealogy on ourselves: eleven ticketed items (C1–C8 calibration battery, E1–E3 engineering gaps) each stated in hypothetico-axiomatic form — hypothesis about our own instrument, formalization, establishment under test, tasks, exit gate. |
...
| [`axis-b-mathematical-review.md`](axis-b-mathematical-review.md) | Whether the scenario matrix (Axis B, six flippable parameters) is mathematically worked out. Verdict: it is a *labeled parameter set*, not yet a mathematical object — no flip is stated as a change to the equations, and no distinguishability requirement exists. Maps each flip onto the model family in this repo, collapses the nominal 2⁶ = 64 cells to a **live 2³ = 8-cell subspace** (P3 × P4 × P6; P1/P2/P5 locked by the Axis A ledger), and defines the build list (scenario schema, 8-cell enumeration, closed-loop `P3` switch on the relaxation ODE, `s(t)` forcing for P6). |
